import { e as bodyLock, f as bodyUnlock, b as bodyLockStatus } from "./common.min.js";
(function polyfill() {
...
const phonePopup = document.querySelector("#phone-input-popup");
if (phonePopup) {
  phonePopup.addEventListener("input", () => {
    let d = phonePopup.value.replace(/\D/g, "");
  ...
  phonePopup.addEventListener("blur", () => {
    const digits = phone.value.replace(/\D/g, "").slice(-10);
    phonePopup.setCustomValidity(digits.length === 10 ? "" : "Please enter a valid US phone number");
  });
}
